...
import { FinancialTransaction, FinancialStatus, PaymentMethod, TransactionType, Invoice } from '@/types';
    ...
    const loadFinanceData = async () => {
      ...
      try {
        ...
        const [txs, invs] = await Promise.all([
          financeService.fetchTransactions(user.schoolId),
          financeService.fetchInvoices(user.schoolId)
        ]);
        setTransactions(txs);
        setInvoices(invs);
      } catch (e) {
        console.error("Failed to sync finance data", e);
      } finally {
        setIsLoading(false);
      }
    };
                ...
                <tbody className="divide-y divide-slate-100">
                  {invoices.map(inv => (
                    <tr key={inv.id} className="hover:bg-blue-50/20 transition-all group">
                      ...
                      <td className="px-6 py-8">
                        <span className={`px-4 py-2 rounded-full text-[8px] font-black uppercase tracking-widest border ${inv.status === 'Paid' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
