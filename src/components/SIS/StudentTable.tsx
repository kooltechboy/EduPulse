   ...
   useEffect(() => {
      const loadStudents = async () => {
         ...
         try {
            ...
            const data = await studentService.fetchAll(user.schoolId);
            setStudents(data);
         } catch (error) {
                  ...
                  <div className={`p-8 md:p-14 bg-gradient-to-br ${selectedStudent.lifecycleStatus === StudentLifecycleStatus.ALUMNI ? 'from-slate-800 to-indigo-950' : 'from-blue-700 to-indigo-900'} text-white relative`}>
                     <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10">
                        ...
                        <div className="text-center md:text-left flex-1">
                           <h3 className="text-3xl md:text-5xl font-black tracking-tighter mb-4 uppercase">{selectedStudent.name}</h3>
                           <div className="flex flex-wrap justify-center md:justify-start gap-3">
                              ...
                              <span className="px-5 py-1.5 bg-blue-600 rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest">Lifecycle: {selectedStudent.lifecycleStatus}</span>
                              <span className="px-5 py-1.5 bg-white/10 rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest border border-white/20">Identity: {selectedStudent.id}</span>
                           </div>
                        </div>
                        <div className="flex gap-4">
                           <button onClick={() => exportPermanentRecord(selectedStudent)} className="p-4 bg-white/10 hover:bg-white/20 rounded-3xl transition-all"><FileDown size={24} /></button>
                  ...
                  <div className="flex-1 overflow-y-auto p-6 md:p-14 scrollbar-hide bg-slate-50/20">
                     ...
                     {activeProfileTab === 'overview' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in duration-500">
                           <div className="bg-white p-10 rounded-[48px] shadow-xl border border-slate-100 space-y-6">
                              <h4 className="text-xl font-black text-slate-900 uppercase">Personal Registry</h4>
                              <div className="grid grid-cols-2 gap-8">
                                 <div>
                                    <p className="text-[10px] font-black text-slate-400 uppercase">Birth Date</p>
                           ...
                           <div className="bg-white p-10 rounded-[48px] shadow-xl border border-slate-100 space-y-6">
                              <h4 className="text-xl font-black text-slate-900 uppercase">Enrollment Data</h4>
                              <div className="space-y-4">
                                 <div className="flex justify-between items-center py-2">
                                    <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Date Enrolled</span>
                                    <span className="font-black text-slate-900 uppercase">{selectedStudent.enrollmentDate || '2023-09-01'}</span>
                                 </div>
                                 <div className="flex justify-between items-center py-2">
                                    <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Attendance Index</span>
                                    <span className="font-black text-blue-600">{selectedStudent.attendance || 0}%</span>
                                 </div>
                                 <div className="flex justify-between items-center py-2">
                                    <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Verified Docs</span>
                                    <span className="font-black text-emerald-600">{selectedStudent.documents?.length || 0} Synced</span>
                                 </div>
                              </div>
                           </div>
                        </div>
