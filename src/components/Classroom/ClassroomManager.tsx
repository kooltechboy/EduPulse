...
import {
   ...
   Baby, School, Globe, UserCheck, Database
} from 'lucide-react';
import {
...
const ClassroomManager: React.FC<{ user: User }> = ({ user }) => {
   ...
   const filteredCourses = useMemo(() => {
      ...
      // Semester filtering
      if (selectedSem !== 'All') {
         list = list.filter(c => c.semester === selectedSem);
      }
      ...
      return list;
   }, [courses, selectedTier, selectedGradeFilter, selectedTeacherFilter, searchQuery, isTeacher, user.id, selectedDept, selectedSem]);
   ...
   const uniqueDepts = useMemo(() => Array.from(new Set(courses.map(c => c.department || 'Unassigned'))), [courses]);
   const uniqueSems = useMemo(() => Array.from(new Set(courses.map(c => c.semester || 'Unassigned'))), [courses]);
   ...
   const renderDashboard = () => (
      <div className="space-y-10 animate-in fade-in duration-700 px-4 md:px-0">
         ...
         <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
            <div>
               <h2 className="text-5xl md:text-7xl font-black text-slate-900 tracking-tighter uppercase leading-none">Classroom<br /><span className="text-blue-600">Learning Node</span></h2>
               <p className="text-slate-500 font-black uppercase text-[10px] tracking-[0.5em] mt-6 flex items-center gap-3">
                  <MonitorPlay size={18} className="text-blue-600" /> {isAdmin ? 'Institutional Observation' : 'Faculty Instruction Station'}
               </p>
            </div>
            {isTeacher && (
