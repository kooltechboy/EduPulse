...
const Timetable: React.FC = () => {
  ...
  const filteredSchedule = MOCK_SCHEDULE.filter(entry => {
  ...
  const uniqueTeachers = Array.from(new Set(MOCK_SCHEDULE.map(s => s.teacher)));
  const uniqueGrades = Array.from(new Set(MOCK_SCHEDULE.map(s => s.grade)));
